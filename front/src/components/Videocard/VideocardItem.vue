<template>
    <div>
        <div class="cpu_item">
            <div class="left-box">
                <div>
                    <img class="cpu_picture" :src="path+videocard_item.image" alt="">
                </div>
                <div>
                    <div class="cpu_name">
                        <a href="">
            ...
            <div>
                <p class="cpu_price">{{ videocard_item.price }}₽</p>
...
export default {
    ...
    methods: {  
        async addToCart() {
            await axios.post('http://127.0.0.1:8000/cart/', {'product_id': this.videocard_item.id}, {headers: {'Authorization': `Bearer ${localStorage.getItem('token')}`}})
            .then((res) => {
                if (res.status === 200) {
