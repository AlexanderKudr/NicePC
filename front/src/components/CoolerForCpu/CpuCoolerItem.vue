<template>
    <div>
        <div class="cpu_item">
            <div class="left-box">
                <div>
                    <img class="cpu_picture" :src="path+cpu_cooler_item.image" alt="">
                </div>
                <div>
                    <div class="cpu_name">
                        <a href="">
                            {{ cpu_cooler_item.name }}
                        </a>
                    </div>
                    <div class="css_line">
                    </div>
            ...
            <div>
                <p class="cpu_price">{{ cpu_cooler_item.price }}₽</p>
...
export default {
    ...
    methods: {  
        async addToCart() {
            await axios.post('http://127.0.0.1:8000/cart/', {'product_id': this.cpu_cooler_item.id}, {headers: {'Authorization': `Bearer ${localStorage.getItem('token')}`}})
            .then((res) => {
                if (res.status === 200) {
